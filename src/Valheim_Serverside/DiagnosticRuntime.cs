    ...
    internal static class DiagnosticRuntime
    {
        ...
        internal static string Status()
        {
            try
            {
                ...
                string result = $"Dedicated Simulation {ServersidePlugin.PluginVersion}; simulation {(Installed ? "installed" : "inactive/rolled back")}; diagnostics {(Configuration.diagnosticsEnabled.Value ? "enabled" : "disabled")}; players {(ZNet.instance ? ZNet.instance.GetPeers().Count : 0)}; {FrameStatus()}; save {save}; {health}";
